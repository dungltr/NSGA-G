...
public class covidProblem4Objectives extends AbstractProblem {
    ////// Parameter
    ...
    public covidProblem4Objectives(List<Double> initVariables, List<Double> realFinalValues, List<Double> parameters,
         ...
         *         int numberOfConstraints = 1;
         */
        super(parameters.size(), realFinalValues.size(), 2);
        //super(4, 4, 1);
        this.numberOfConstraints = 1;
        this.numberOfVariables = parameters.size(); // consider parameters are variable in the problem
        this.numberOfObjectives = realFinalValues.size(); // consider the error of final values are the objectives
        ...
        for (int i = 0; i < realFinalValues.size(); i++){
            this.realFinalValues.add(realFinalValues.get(i));
        }

        //System.out.println("The size of InitVariables: "+ this.initVariables.size());
        //System.out.println("The size of Variables (size of parameters): "+ this.parameters.size());
        //System.out.println("The size of realFinalValues: "+ this.realFinalValues.size());
    ...
    public void evaluate(Solution solution) {
        ...
        List<Double> estimateValues = realValues4Objectives(finalVariables);
        ...
        if ((check)&&(R0_1 < this.Rlimit.get(0))&&(R0_1 > underLimit) &&(R0_2 < this.Rlimit.get(1))){
            for (int i = 0; i < realFinalValues.size(); i++){
    ...
    public Solution newSolution() {
        ...
        */
        /*
        for (int i = zetaIndex; i < numberOfVariables; i++){
        ...
        */
        ...
        double alpha = ((RealVariable)solution.getVariable(alfaIndex)).getValue();
        solution.setVariable(1, new RealVariable(0, alpha));
        solution.setVariable(2, new RealVariable(0, alpha));
        solution.setVariable(3, new RealVariable(0, alpha));

        solution.setVariable(4, new RealVariable(0, 1));
        double epsilon = ((RealVariable)solution.getVariable(epsilonIndex)).getValue();
        solution.setVariable(5, new RealVariable(epsilon, 1));
        /*
        solution.setVariable(0, new RealVariable(0, 1));
